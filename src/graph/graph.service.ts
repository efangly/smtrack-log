...
export class GraphService {
  ...
  async findAll(sn: string, filter: string, freq: string) {
    ...
    const cache = await this.redis.get(`graph:${sn}${filter.split(',').join("")}${freq || ""}`);
    ...
    if (result.length > 0) await this.redis.set(`graph:${sn}${filter.split(',').join("")}${freq || ""}`, JSON.stringify(result), 1800);
    return result;
  }
